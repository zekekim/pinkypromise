...
import 'package:pinkypromise/extensions/firebase_firestore_extension.dart';
...
class ItemRepository implements BaseItemRepository {
  ...
  Future<List<Item>> retrieveItems({required String userId}) async {
    try {
      final snap = await _read(firestoreProvider).usersListRef(userId).get();
  ...
  Future<void> deleteItem(
    ...
    try {
      await _read(firestoreProvider).usersListRef(userId).doc(itemId).delete();
    } on FirebaseException catch (e) {
      throw CustomException(message: e.message);
    }
  }
